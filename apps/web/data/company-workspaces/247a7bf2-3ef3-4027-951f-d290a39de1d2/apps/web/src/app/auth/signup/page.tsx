...
export default function SignupPage() {
  ...
  const [role, setRole] = useState<'solo-builder' | 'team-lead' | 'enterprise-admin'>('solo-builder');
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // Store user preferences in sessionStorage for onboarding flow
        sessionStorage.setItem('userOnboarding', JSON.stringify({ 
          role, 
          name,
          email 
        }));
  ...
  const handleGithubSignup = async () => {
    window.location.href = '/api/auth/github/login?callbackUrl=/api/auth/github/callback';
  };
          ...
          {!showForm ? (
            ...
            <>
              <button
                ...
                className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border-2 border-gray-200 hover:border-gray-300 rounded-xl transition-all duration-200 mb-6 group"
              >
              ...
              <button
                onClick={() => window.location.href = '/api/auth/google/login?callbackUrl=/api/auth/google/callback'}
                className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border-2 border-gray-200 hover:border-gray-300 rounded-xl transition-all duration-200 mb-6 group"
              >
              ...
              <form onSubmit={handleEmailSignup} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {[
                      { value: 'solo-builder', label: 'Solo Builder', description: 'Quick workflow automation' },
                      { value: 'team-lead', label: 'Team Lead', description: 'Organize team work and projects' },
                      { value: 'enterprise-admin', label: 'Enterprise Admin', description: 'Scale with control and governance' },
                    ].map((option) => (
                      <button
                        key={option.value}
                        type="button"
